...
export function buildTimeline() {
  ...
  svg.innerHTML = `
    <defs>
      <linearGradient id="archGradLotr" x1="0%" y1="0%" x2="100%" y2="0%">
        <stop offset="0%"   stop-color="rgba(245,158,11,0.9)"/>
        <stop offset="25%"  stop-color="rgba(59,130,246,0.7)"/>
        <stop offset="50%"  stop-color="rgba(14,165,233,0.7)"/>
        <stop offset="75%"  stop-color="rgba(148,163,184,0.7)"/>
        <stop offset="100%" stop-color="rgba(34,197,94,0.9)"/>
      </linearGradient>
      <filter id="archGlowLotr" x="-10%" y="-50%" width="120%" height="200%">
        <feGaussianBlur in="SourceGraphic" stdDeviation="6"/>
      </filter>
    </defs>
    <path d="${pathD}" fill="none" stroke="url(#archGradLotr)"
          stroke-width="10" filter="url(#archGlowLotr)" opacity="0.4"
          id="lotrSpineGlow"/>
    <path d="${pathD}" fill="none" stroke="url(#archGradLotr)"
          stroke-width="2.5" id="lotrSpine"
          stroke-dasharray="10000" stroke-dashoffset="10000"/>
  `;
  wrap.appendChild(svg);

  gsap.to('#lotrSpine', { strokeDashoffset: 0, duration: 2.8, ease: 'power1.inOut', delay: 0.2 });
  gsap.to('#lotrSpineGlow', { opacity: 0.4, duration: 2.8, ease: 'power1.inOut', delay: 0.2 });
